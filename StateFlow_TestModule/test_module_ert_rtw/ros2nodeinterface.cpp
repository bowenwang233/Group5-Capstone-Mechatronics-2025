...
// C/C++ source code generated on : Tue Oct 28 14:49:19 2025
//
#ifdef _MSC_VER
#pragma warning(push)
...
#include <chrono>
#include <utility>
const std::string SLROSNodeName("test_module");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("test_module");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"test_module\" **\n");
		...
		rtmSetErrorStatus(test_module_M, (NULL));
		...
		rtExtModeCheckInit(1);
		{
			...
			rtExtModeWaitForStartPkt(test_module_M->extModeInfo, 1,
				&rtmStopReq);
			if (rtmStopReq) {
...
boolean_T NodeInterface::getStopRequestedFlag(void) {
    ...
    return (!(rtmGetErrorStatus(test_module_M) 
        == (NULL)));
    #else
    return (!(rtmGetErrorStatus(test_module_M) 
        == (NULL)) || rtmGetStopRequested(test_module_M));
    #endif
}
...
void NodeInterface::baseRateTask(void) {
  mRunModel = (rtmGetErrorStatus(test_module_M) ==
              (NULL));
  while (mRunModel) {
    mBaseRateSem.wait();
#ifdef MW_DEBUG_LOG
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModePauseIfNeeded(test_module_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(test_module_M, true);
      }
...
void NodeInterface::extmodeBackgroundTask(void)
{
  while (mRunModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(test_module_M->extModeInfo, 1,
                       &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(test_module_M, true);
      }
