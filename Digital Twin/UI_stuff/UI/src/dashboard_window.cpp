...
DashboardWindow::DashboardWindow() {
    ...
    stateLayout->addWidget(makeStateItem("DISTANCE", "--", &personDistValue_));
...
QObject::connect(receiver_.get(), &UdpReceiver::personDistanceReceived,
    this,
    [this](float dist) {

        if (!personDistValue_) return;

        if (std::isnan(dist)) {
            personDistValue_->setText("No person");
        } else {
            personDistValue_->setText(QString::number(dist, 'f', 2) + " m");
        }
});
}
